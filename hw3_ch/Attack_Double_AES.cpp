...
//plaintext: a619985610a4d9d00174ac891013dbe2
	uint8_t in[] = {
		0xa6, 0x19, 0x98, 0x56,
		0x10, 0xa4, 0xd9, 0xd0,
		0x01, 0x74, 0xac, 0x89,
		0x10, 0x13, 0xdb, 0xe2};
//ciphertext: 3e60b9afaee343e45dd57003521ab3ea
	uint8_t ct_out[] = {
		0x3e, 0x60, 0xb9, 0xaf,
		0xae, 0xe3, 0x43, 0xe4,
		0x5d, 0xd5, 0x70, 0x03,
	...
	for (int i = 0; i < 256; i++){
		...
		for(int j = 0; j < 256; j++){
			...
			for(int k = 0; k < 256; k++){
				...
				long count = 65536 * i + 256 * j + k;
				ciphertext_k1.insert(pair<string, long>(c_k1, count));
				printf("Attack Times: %ld \n", count);
			}
		}
	}
	...
	for (int i = 0; i < 256; i++){
		...
		for(int j = 0; j < 256; j++){
			...
			for(int k = 0; k < 256; k++){
				...
				key_expansion(key, w);
				inv_cipher(ct_out, ct_in, w);

				for (int m = 0; m < 16; m++){
					p_k2 += ct_in[m];
				}
				it = ciphertext_k1.find(p_k2);
				if (it != ciphertext_k1.end()){
					cout << "K1:" << it->second << endl;
					cout << "K2:" << 65536 * i + 256 * j + k << endl;
				}
			}
		}
	}
